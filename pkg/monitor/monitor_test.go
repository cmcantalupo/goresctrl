...
func TestEnsureGroup_CanonicalizesKey(t *testing.T) {
	...
	// Membership and removal work via either form.
	mgr.AddMember(compact, "ctr-1")
	assert.Equal(t, 1, mgr.MemberCount(dashed))
...
// --- Member bookkeeping tests (Task 2.5) ---

func TestMemberBookkeeping(t *testing.T) {
	tmpDir := t.TempDir()
	mgr, err := New(Options{ResctrlRoot: tmpDir})
	require.NoError(t, err)
	...
	assert.Equal(t, 0, mgr.MemberCount("pod-uid-1"))

	// Add two containers.
	mgr.AddMember("pod-uid-1", "ctr-a")
	assert.Equal(t, 1, mgr.MemberCount("pod-uid-1"))
	mgr.AddMember("pod-uid-1", "ctr-b")
	assert.Equal(t, 2, mgr.MemberCount("pod-uid-1"))
	...
	// Remove one.
	mgr.RemoveMember("pod-uid-1", "ctr-a")
	assert.Equal(t, 1, mgr.MemberCount("pod-uid-1"))

	// Remove last.
	mgr.RemoveMember("pod-uid-1", "ctr-b")
	assert.Equal(t, 0, mgr.MemberCount("pod-uid-1"))
}

func TestMemberCount_UntrackedKey(t *testing.T) {
	tmpDir := t.TempDir()
	mgr, err := New(Options{ResctrlRoot: tmpDir})
	require.NoError(t, err)

	// Untracked key returns 0 (no panic).
	assert.Equal(t, 0, mgr.MemberCount("no-such-key"))

	// AddMember/RemoveMember on untracked key are no-ops.
	mgr.AddMember("no-such-key", "ctr-x")
	mgr.RemoveMember("no-such-key", "ctr-x")
}
...
func TestReconcile_IgnoresNonKeyDirs(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(monDir, 0755))
	// Special dirs that should never be removed.
	for _, special := range []string{"info", "mon_L3_00"} {
		require.NoError(t, os.Mkdir(filepath.Join(monDir, special), 0755))
	}
	...
	for _, special := range []string{"info", "mon_L3_00"} {
		_, err = os.Stat(filepath.Join(monDir, special))
		assert.NoError(t, err, "should not remove %s", special)
	}
}
